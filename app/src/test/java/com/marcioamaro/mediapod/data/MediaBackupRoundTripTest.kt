...
import com.marcioamaro.mediapod.util.BackupCryptoHelper
...
class MediaBackupRoundTripTest {
    @Before fun resetApplicationSingletons() {
        for (type in listOf(PodcastRepository::class.java, IpodPreferencesManager::class.java,
                MediaLibraryRepository::class.java, com.marcioamaro.mediapod.data.db.RadioDatabase::class.java)) {
    ...
    @Test fun encryptedBackupRestoresPlayedEpisodesYoutubeAndCollectionsIdempotently() = runBlocking {
        ...
        com.marcioamaro.mediapod.util.DataUsagePolicy(context).remoteArtwork = true
        com.marcioamaro.mediapod.util.DataUsagePolicy(context).preferredBitrate = 0
        ...
        assertFalse(com.marcioamaro.mediapod.util.DataUsagePolicy(context).remoteArtwork)
        assertEquals(64000, com.marcioamaro.mediapod.util.DataUsagePolicy(context).preferredBitrate)
