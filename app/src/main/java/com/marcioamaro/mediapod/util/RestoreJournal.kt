...
object RestoreJournal {
    ...
    suspend fun apply(context: Context, operation: suspend () -> Boolean): Boolean = lock.withLock {
        com.marcioamaro.mediapod.data.repository.MediaLibraryRepository.getInstance(context).withStorageLock {
        ...
        try {
            ...
            withContext(NonCancellable) {
                recoverLocked(context)
                com.marcioamaro.mediapod.data.repository.PodcastRepository.getInstance(context).reloadFromStorage()
                com.marcioamaro.mediapod.data.repository.MediaLibraryRepository.getInstance(context).reloadFromStorage()
            }
