    ...
    val import = rememberLauncherForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
        if (uri != null) scope.launch {
            ...
            message = try {
                ...
                context.getString(com.marcioamaro.mediapod.R.string.opml_added, count)
            } catch (cancelled: kotlinx.coroutines.CancellationException) { throw cancelled }
            catch (_: Exception) { context.getString(com.marcioamaro.mediapod.R.string.opml_invalid) }
            finally { busy = false }
        }
    }
    val export = rememberLauncherForActivityResult(ActivityResultContracts.CreateDocument("text/x-opml")) { uri ->
        if (uri != null) scope.launch {
            message = try {
                ...
                context.getString(com.marcioamaro.mediapod.R.string.opml_exported)
            } catch (_: Exception) { context.getString(com.marcioamaro.mediapod.R.string.file_save_error) }
        }
    }
    TextButton(enabled = !busy, onClick = { import.launch(arrayOf("text/*", "application/xml", "application/octet-stream")) }) { Text(context.getString(com.marcioamaro.mediapod.R.string.opml_import)) }
    TextButton(enabled = !busy, onClick = { export.launch("mediapod-assinaturas.opml") }) { Text(context.getString(com.marcioamaro.mediapod.R.string.opml_export)) }
    message?.let { Text(it) }
}
