...
fun PersonalLibraryScreen(viewModel: RadioViewModel) {
    ...
    val downloads = remember { com.example.data.download.PodcastDownloadManager.getInstance(context) }
    ...
    LazyColumn(Modifier.fillMaxSize().padding(8.dp)) {
        item {
            ...
            listOf(context.getString(com.example.R.string.library_continue), context.getString(com.example.R.string.library_bookmarks), context.getString(com.example.R.string.library_subscriptions), context.getString(com.example.R.string.library_smart), context.getString(com.example.R.string.library_discover)).forEachIndexed { index, label ->
        ...
        if (tab == 0) {
            ...
            item { Text(if (loading) context.getString(com.example.R.string.library_loading) else context.getString(com.example.R.string.library_resume_help)) }
            ...
            items(library.recents.filter { media.position(it) > 0 }, key = { it }) { key ->
                audioByKey[key]?.let { track ->
                    TextButton(onClick = { viewModel.playLocalAudio(track, audio); viewModel.navigateTo(IpodScreenDestination.MP3_NOW_PLAYING) }) { Text("${context.getString(com.example.R.string.menu_mp3)} · ${track.title}") }
                }
                videoByKey[key]?.let { track ->
                    TextButton(onClick = { viewModel.playLocalVideo(track); viewModel.navigateTo(IpodScreenDestination.VIDEO_PLAYER) }) { Text("${context.getString(com.example.R.string.menu_video)} · ${track.title}") }
                }
            }
        } else if (tab == 1) {
            if (notes.isEmpty()) item { Text(context.getString(com.example.R.string.bookmark_empty)) }
            items(notes, key = { it.id }) { note ->
                TextButton(onClick = { playEpisode(note.episode, note.positionMs) }) { Text("${note.episode.title} · ${note.positionMs / 1000}s\n${note.note}") }
                Row {
                    TextButton(onClick = { editing = note }) { Text(context.getString(com.example.R.string.feature_edit)) }
                    TextButton(onClick = { bookmarks.delete(note.id) }) { Text(context.getString(com.example.R.string.feature_delete)) }
                }
            }
        } else if (tab == 2) {
            ...
            items(subscriptions, key = { it.id }) { show ->
                ...
                TextButton(onClick = { podcasts.unsubscribe(show.id) }) { Text(context.getString(com.example.R.string.subscription_remove)) }
            }
        } else if (tab == 3) {
            item {
                Text(context.getString(com.example.R.string.smart_scope))
                LibrarySuggestions.Mode.entries.zip(listOf(context.getString(com.example.R.string.smart_unplayed), context.getString(com.example.R.string.smart_downloaded), context.getString(com.example.R.string.smart_favorites), context.getString(com.example.R.string.smart_short))).forEach { (value, label) ->
                    TextButton(onClick = { mode = value; preferences.edit().putString("smart_mode", value.name).apply() }) { Text(if (mode == value) "• $label" else label) }
                }
                Text(context.getString(com.example.R.string.smart_duration, maxMinutes))
                ...
                Text(context.getString(com.example.R.string.smart_limit, limit))
                ...
                TextButton(enabled = smart.isNotEmpty(), onClick = { playEpisode(smart.first(), queue = smart) }) { Text(context.getString(com.example.R.string.smart_play)) }
            }
            items(smart, key = { it.id }) { episode -> TextButton(onClick = { playEpisode(episode, queue = smart) }) { Text(episode.title) } }
        } else {
            item {
                PreferenceToggle(context.getString(com.example.R.string.discovery_enable), discovery) { discovery = it; preferences.edit().putBoolean("discovery_enabled", it).apply() }
                PreferenceToggle(context.getString(com.example.R.string.discovery_history), useHistory) { useHistory = it; preferences.edit().putBoolean("discovery_history", it).apply() }
                OutlinedTextField(value = interests, onValueChange = { interests = it.take(300); preferences.edit().putString("discovery_interests", interests).apply() }, label = { Text(context.getString(com.example.R.string.discovery_interests)) })
                Text(context.getString(com.example.R.string.discovery_privacy))
                TextButton(onClick = {
                ...
                }) { Text(context.getString(com.example.R.string.discovery_clear)) }
            }
