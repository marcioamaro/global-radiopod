        ...
        if (uri != null) {
            ...
            scope.launch {
                message = try {
                    ...
                    context.getString(com.example.R.string.catalog_imported)
                } catch (cancelled: kotlinx.coroutines.CancellationException) { throw cancelled }
                catch (_: Exception) { context.getString(com.example.R.string.catalog_invalid) }
                finally { busy = false }
            }
        }
    }
    TextButton(enabled = !busy, onClick = { picker.launch(arrayOf("application/zip", "application/octet-stream")) }) {
        Text(if (busy) context.getString(com.example.R.string.catalog_validating) else context.getString(com.example.R.string.catalog_import))
    }
    message?.let { Text(it) }
}
