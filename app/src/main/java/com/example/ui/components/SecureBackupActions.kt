...
fun rememberSecureBackupActions(onResult: (Boolean, String) -> Unit): BackupActions {
    ...
    val export = rememberLauncherForActivityResult(ActivityResultContracts.CreateDocument("application/octet-stream")) { uri ->
        ...
        if (uri != null && key != null) scope.launch {
            try {
                ...
                onResult(ok, if (ok) context.getString(com.example.R.string.backup_saved) else context.getString(com.example.R.string.backup_save_error))
            } finally { key.fill('\u0000') }
        } else key?.fill('\u0000')
    }
    ...
    if (mode != null) AlertDialog(
        ...
        title = { Text(if (mode == "export") context.getString(com.example.R.string.backup_protect) else context.getString(com.example.R.string.backup_restore)) },
        text = {
            Column(Modifier.verticalScroll(rememberScrollState())) {
                Text(if (mode == "export") context.getString(com.example.R.string.backup_password_help)
                    else context.getString(com.example.R.string.backup_legacy_help))
                OutlinedTextField(password, { password = it }, label = { Text(context.getString(com.example.R.string.backup_password)) },
                    visualTransformation = PasswordVisualTransformation(), singleLine = true)
                if (mode == "export") OutlinedTextField(confirmation, { confirmation = it }, label = { Text(context.getString(com.example.R.string.backup_confirm)) },
                    visualTransformation = PasswordVisualTransformation(), singleLine = true)
            }
        },
        confirmButton = {
                ...
                if (exporting) {
                    ...
                    if (uri != null) scope.launch {
                        try {
                            ...
                            onResult(ok, if (ok) context.getString(com.example.R.string.backup_restored) else context.getString(com.example.R.string.backup_restore_error))
                        } finally { key.fill('\u0000') }
                    } else key.fill('\u0000')
                }
            }) { Text(context.getString(com.example.R.string.library_continue)) }
        },
        dismissButton = { TextButton(onClick = { mode = null; password = ""; confirmation = ""; pendingUri = null }) { Text(context.getString(com.example.R.string.feature_cancel)) } }
    )
